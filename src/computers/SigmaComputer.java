...
public class SigmaComputer
{
	...
	public SigmaComputer(ArrayList<RankedData> piVector, ArrayList<RankedData> sigmaVector, QVector qVector)
	{
	...
	public ArrayList<RankedData> getSigmaValues(RankedData sigma) 
	{
		...
		sigma.negate(sigmaSize-1);
		sigmaValues.add(sigma);
	...
	public boolean computeSVector()
	{
		...
		for (int s=0; s<sigmaVector.size(); s++)
		{
			...
			ArrayList<RankedData> testValues = getSigmaValues(sigmaVector.get(s));
			
			for (int i=0; i<testValues.size(); s++)
			{
				double sum = 0;
				for (int j=0; j<piVector.size(); j++) 
				{
					sum += qVector.get(s, j)*distanceRanker.getDistance(piVector.get(j), testValues.get(i));
				}
				if (i == 0)
				{
					minValue = sum;
